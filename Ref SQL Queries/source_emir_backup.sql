...
CREATE PROC [dbo].[spa_source_emir]
	@sub_id VARCHAR(8000) = NULL,
	@stra_id VARCHAR(8000) = NULL,
	@book_id VARCHAR(8000) = NULL,
	...
	@commodity_id VARCHAR(8000) = NULL,
...
DECLARE @sub_id VARCHAR(8000) = NULL,
		@stra_id VARCHAR(8000) = NULL,
		@book_id VARCHAR(8000) = NULL,
		@sub_book_id VARCHAR(8000) = NULL,
		@create_date_from VARCHAR(100) = NULL,
...
SELECT @sub_book_id=NULL,@sub_id=NULL,@stra_id=NULL,@book_id=NULL,@create_date_from='2017-12-02',@create_date_to='2018-01-02',@deal_id=NULL,@counterparty_id=NULL,@contract_id=NULL,@commodity_id=NULL,@flag='i',@submission_type='44704',@action_type='N',@level='P',@action_type_mifid='',@level_mifid='X',@deal_ref_id='1508394387043021000'
--*****************Test Code Start*******************/
SET NOCOUNT ON
...
DECLARE @_sql VARCHAR(MAX), @error_spa VARCHAR(1000), @temp_path VARCHAR(500)
...
SET @_sql ='
	INSERT INTO #books(sub_id, stra_id, book_id, sub, stra, book, source_system_book_id1, source_system_book_id2, source_system_book_id3, source_system_book_id4, logical_name, sub_book_id, counterparty_id)
...
SET @_sql = @_sql +  ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @sub_id + ''') scsv_sub on scsv_sub.item = sub.entity_id'

IF @stra_id IS NOT NULL
	SET @_sql = @_sql + ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @stra_id + ''') scsv_stra on scsv_stra.item = stra.entity_id'
	
IF @book_id IS NOT NULL
	SET @_sql = @_sql + ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @book_id + ''') scsv_book on scsv_book.item = book.entity_id'

IF @sub_book_id IS NOT NULL
	SET @_sql = @_sql + ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @sub_book_id + ''') scsv_sub_book on scsv_sub_book.item = ssbm.book_deal_type_map_id'
...
BEGIN
	...
	*******************************************/
	IF @process_id IS NULL
		SET @process_id = REPLACE(LOWER(NEWID()), '-', '_')
	...
	SET @_sql = '
		...
		SELECT DISTINCT sdh.source_deal_header_id, sdh.deal_id, sdh.template_id, sdh.counterparty_id, books.sub_book_id, sdh.deal_date, sdh.physical_financial_flag,
						sdh.entire_term_start, sdh.entire_term_end, sdh.source_deal_type_id, sdh.deal_sub_type_type_id, sdh.option_flag, sdh.option_type, sdh.option_excercise_type, 
						sdh.header_buy_sell_flag, sdh.create_ts, sdh.update_ts, sdh.internal_desk_id, sdh.product_id, sdh.commodity_id, sdh.block_define_id, sdh.deal_status, 
						sdh.description1, sdh.description2, sdh.trader_id, sdh.contract_id, 0, sdh.ext_deal_id, sdv.code
		...
		LEFT JOIN static_data_value sdv
			ON sdv.value_id = sdh.confirm_status_type
		'

	IF @counterparty_id IS NOT NULL
		SET @_sql = @_sql + ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @counterparty_id + ''') scsv 
			ON scsv.item = sdh.counterparty_id'

	IF @commodity_id IS NOT NULL
		SET @_sql = @_sql + ' INNER JOIN dbo.splitcommaseperatedvalues(''' + @commodity_id + ''') scsv2 
			ON scsv2.item = spcd.commodity_id'
	
	SET @_sql = @_sql + ' INNER JOIN #books books 
	...
	IF @create_date_from IS NOT NULL AND @create_date_to IS NOT NULL
		SET @_sql = @_sql + ' AND (CONVERT(VARCHAR(10), sdh.create_ts,120) BETWEEN '''+ CAST(@create_date_from AS VARCHAR(18)) + ''' AND '''+ CAST(@create_date_to AS VARCHAR(18))+ ''' OR 
								   CONVERT(VARCHAR(10), sdh.update_ts,120) BETWEEN '''+ CAST(@create_date_from AS VARCHAR(18)) + ''' AND '''+ CAST(@create_date_to AS VARCHAR(18))+ ''')'
	
	IF @deal_id IS NOT NULL
		SET @_sql = @_sql + ' AND sdh.source_deal_header_id IN ('+ @deal_id +') '
	
	IF @deal_ref_id IS NOT NULL
		SET @_sql = @_sql + ' AND sdh.deal_id LIKE ''%' + @deal_ref_id + '%''' 
	
	EXEC(@_sql)
	
	CREATE INDEX indx_deal_id ON #temp_deals (source_deal_header_id, sub_book_id)
	...
	SELECT td.source_deal_header_id, sdd.source_deal_detail_id, sdd.term_start, sdd.term_end, sdd.leg, sdd.fixed_float_leg, sdd.buy_sell_flag, sdd.curve_id, sdd.location_id, sdd.physical_financial_flag, 
			sdd.deal_volume, sdd.total_volume, sdd.standard_yearly_volume, sdd.deal_volume_frequency, sdd.deal_volume_uom_id, sdd.multiplier, sdd.volume_multiplier2, sdd.fixed_price, 
	...
	CREATE INDEX indx_deal_detail_id ON #temp_deals (source_deal_header_id)
	...
	INSERT INTO #temp_cpty_udf_values(source_deal_header_id, counterparty_id, [ACER], [BIC], [Collateralization], [Commercial/Treasury], [Corporate Sector], [CSA Reportable Trade], [EAN Gas], [EAN Power], [EEA], [EIC], [Financial/Non-Financial], [LEI], [PRP Gas], [PRP Power], [TSO Gas], [TSO power], [Collateral Portfolio Code])
	SELECT source_deal_header_id, counterparty_id, [ACER], [BIC], [Collateralization], [Commercial/Treasury], [Corporate Sector], [CSA Reportable Trade], [EAN Gas], [EAN Power], [EEA], [EIC], [Financial/Non-Financial], [LEI], [PRP Gas], [PRP Power], [TSO Gas], [TSO power], [Collateral Portfolio Code]
	FROM (
	...
	PIVOT(MAX(a.static_data_udf_values) FOR a.Field_label IN ([ACER], [BIC], [Collateralization], [Commercial/Treasury], [Corporate Sector], [CSA Reportable Trade], [EAN Gas], [EAN Power], [EEA], [EIC], [Financial/Non-Financial], [LEI], [PRP Gas], [PRP Power], [TSO Gas], [TSO power], [Collateral Portfolio Code])) AS P
	
	INSERT INTO #temp_cpty_udf_values (sub_book_id, counterparty_id, [ACER], [BIC], [Collateralization], [Commercial/Treasury], [Corporate Sector], [CSA Reportable Trade], [EAN Gas], [EAN Power], [EEA], [EIC], [Financial/Non-Financial], [LEI], [PRP Gas], [PRP Power], [TSO Gas], [TSO power], [Collateral Portfolio Code])		
	SELECT sub_book_id, counterparty_id, [ACER], [BIC], [Collateralization], [Commercial/Treasury], [Corporate Sector], [CSA Reportable Trade], [EAN Gas], [EAN Power], [EEA], [EIC], [Financial/Non-Financial], [LEI], [PRP Gas], [PRP Power], [TSO Gas], [TSO power], [Collateral Portfolio Code]
	FROM (
	...
	SELECT source_deal_header_id, [Asset Class], [Product Classification Type], [Product Classification], [Global UTI], [Venue of Execution], [Execution Timestamp], [Clearing Obligation], [Cleared], [Clearing Timestamp], [Intragroup], [Trading Venue Transaction ID], [Derivative Notional], [Country of the Branch Membership], [Initial Margin], [Complex Trade Component ID], [Instrument Full Name], [Instrument Classification], [Delivery Type], [Firm Execution], [Short Selling Indicator], [OTC Post-Trade Indicator], [Commodity Derivative Indicator], [Publication Time Stamp], [Venue of Publication], [Flags], [Supplimentary Deferral Flags], [Threshold], [Decision Maker ID]
	FROM (
	...
	PIVOT (MAX(a.udf_value) FOR a.Field_label IN ([Asset Class], [Product Classification Type], [Product Classification], [Global UTI], [Venue of Execution], [Execution Timestamp], [Clearing Obligation], [Cleared], [Clearing Timestamp], [Intragroup], [Trading Venue Transaction ID], [Derivative Notional], [Country of the Branch Membership], [Initial Margin], [Complex Trade Component ID], [Instrument Full Name], [Instrument Classification], [Delivery Type], [Firm Execution], [Short Selling Indicator], [OTC Post-Trade Indicator], [Commodity Derivative Indicator], [Publication Time Stamp], [Venue of Publication], [Flags], [Supplimentary Deferral Flags], [Threshold], [Decision Maker ID])) AS p
	...
	BEGIN
		...
		SELECT DISTINCT source_deal_header_id = sdh.source_deal_header_id, -- done
						deal_id = MAX(sdh.deal_id),-- done
						sub_book_id = MAX(sdh.sub_book_id),-- done
						reporting_timestamp = CONVERT(VARCHAR(10), CONVERT(DATETIME, GETDATE(), 103), 126) + 'T' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(8)) + 'Z',-- done
						counterparty_id = MAX(sub_cpty.LEI),--done
						other_counterparty_id = 'LEI',--done
						counterparty_name = MAX(deal_cpty.LEI),--done
						--counterparty_country = IIF(MAX(deal_cpty.LEI) IS NULL, MAX(sdv_country.code), NULL),
						counterparty_country = MAX(sdv_country.code),
						...
						broker_id = NULL,
						reporting_entity_id = MAX(sub_cpty.LEI),--done
						clearing_member_id = 'MLDY5N6PZ58ZE60QU102', --LEI code of DTCC
						beneficiary_type_id = 'LEI',
						beneficiary_id = MAX(deal_cpty.LEI),
						trading_capacity = 'P',
						counterparty_side = UPPER(MAX(sdh.header_buy_sell_flag)),
						commercial_or_treasury = '',--MAX(sub_cpty.[Commercial/Treasury]),
						clearing_threshold = 'N',
						--contarct_mtm_value = IIF(MAX(sco.counterparty_id) = 'ICE', NULL, SUM(sdpd.contract_value)),
						contarct_mtm_value = SUM(ABS(sdpd.und_pnl)),
						contarct_mtm_currency = MAX(sc.currency_name),
						--valuation_ts = IIF(MAX(sco.counterparty_id) = 'ICE', NULL, CONVERT(VARCHAR(10), MAX(ISNULL(sdp.update_ts, sdp.create_ts)), 120)),
						valuation_ts = CONVERT(VARCHAR(10), MAX(ISNULL(sdp.update_ts, sdp.create_ts)), 120),
						--valuation_type = IIF(MAX(sco.counterparty_id) = 'ICE', 'C', 'M'),
						valuation_type = 'M',
						collateralization = 'FC',--MAX(sub_cpty.Collateralization),
						...
						initial_margin_posted_currency = 'EUR',
						variation_margin_posted = MAX(deal_udf.[Threshold]),
						variation_margin_posted_currency = 'EUR',
						initial_margin_received = SUM(ipi.payment_amount),
						initial_margin_received_currency = 'EUR',
						...
						contract_type = CASE WHEN MAX(sdt.source_deal_type_name) = 'Future' THEN 'FU' 
											 ...
											 ELSE 'OT' 
										END,
						asset_class = 'CO',--MAX(deal_udf.[Asset Class]),
						...
						product_identification_type = 'I',
						product_identification = MAX(gmv1.clm1_value),
						underlying = 'I',
						underlying_identification = MAX(gmv1.clm4_value),
						...
						trade_id = MAX(sdh.deal_id), --MAX(deal_udf.[Global UTI]),
						report_tracking_no = NULL,
						complex_trade_component_id = NULL,
						exec_venue = 'XOFF',--MAX(deal_udf.[Venue of Execution]),
						[compression] = 'N',
						...
						notional_amount = ISNULL(SUM(ABS(sdpd.und_pnl)), CAST((ISNULL(SUM(sdd.total_volume), 0) * ISNULL(AVG(sdd.fixed_price), 0)) AS NUMERIC(38, 20))),
						price_multiplier = CASE WHEN MAX(scm.commodity_id) IN ('CER', 'EUA', 'ERU', 'EUAA') THEN 1000 
												WHEN MAX(scm.commodity_id) = 'Gasoil' THEN 100
												WHEN MAX(scm.commodity_id) = 'Rbob' THEN 42000
												ELSE MAX(sdd.multiplier)
											  END ,
						quantity = SUM(ISNULL(sdd.deal_volume, 0)),
						...
						termination_date = NULL,--CONVERT(VARCHAR(10), CONVERT(DATETIME, GETDATE(), 103), 126),
						settlement_date = NULL,--CONVERT(VARCHAR(10), CONVERT(DATETIME, GETDATE(), 103), 126),
						...
						confirm_means = 'Y',--MAX(sdh.confirm_status),
						clearing_obligation = 'N',
						cleared = 'N', --MAX(deal_udf.Cleared),
						clearing_ts = NULL,--MAX(deal_udf.[Execution Timestamp]),--CONVERT(VARCHAR(10), CONVERT(DATETIME,  MAX(sdh.deal_date), 103),126) + 'T' + CAST(CAST( MAX(sdh.deal_date) AS TIME) AS VARCHAR(8)) + 'Z',
						ccp = NULL,--MAX(deal_cpty.LEI),
						intra_group = 'Y',
						fixed_rate_leg_1 = NULL,
						fixed_rate_leg_2 = NULL,
						...
						exchange_rate_1 = NULL,
						forward_exchange_rate = NULL,
						exchange_rate_basis = NULL,
						commodity_base = 'EV',--CASE WHEN MAX(deal_udf.[Asset Class]) = 'CO' THEN 'EV' ELSE NULL END,
						commodity_details = 'EM',--CASE WHEN (CASE WHEN MAX(deal_udf.[Asset Class]) = 'CO' THEN 'EV' ELSE NULL END) = 'EV' THEN 'EM' ELSE 'OT' END,
						delivery_point = NULL,
						interconnection_point = NULL,
						load_type = NULL,
									--CASE 
									--	WHEN ISNULL(MAX(internal_desk_id), 17300) = 17302
									--		THEN 'SH'
									--	WHEN MAX(sdv_block.code) LIKE '%Base%'
									--		THEN 'BL'
									--	WHEN MAX(sdv_block.code) LIKE '%Peak%'
									--		THEN 'PL'
									--	WHEN MAX(sdv_block.code) LIKE '%Offpeak%'
									--		THEN 'OP'
									--	ELSE 'OT'
									--END,
						load_delivery_interval = NULL,--'T' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(8)) + 'Z',
						delivery_start_date = NULL,--CONVERT(VARCHAR(10), CONVERT(DATETIME, MAX(sdh.entire_term_start), 103),126) + 'T' + CAST(CAST(MAX(sdh.entire_term_start) AS TIME) AS VARCHAR(8)) + 'Z',
						delivery_end_date = NULL,--CONVERT(VARCHAR(10), CONVERT(DATETIME, MAX(sdh.entire_term_end), 103),126) + 'T' + CAST(CAST(MAX(sdh.entire_term_start) AS TIME) AS VARCHAR(8)) + 'Z',
						duration = NULL,
						days_of_the_week = NULL,
						delivery_capacity = NULL,--SUM(sdd.deal_volume),
						quantity_unit = NULL,--MAX(su.uom_name),
						price_time_interval_quantity = NULL,--AVG(sdd.fixed_price),
						...
						action_type = CASE WHEN MAX(deal_status.code) IN ('New', 'Amended') THEN 'N' 
										   WHEN MAX(deal_status.code) = 'Cancelled' THEN 'C' 
									  END ,
						[level] = @level,
						report_type = NULL,
		...
		LEFT JOIN source_deal_pnl sdp
			ON sdp.source_deal_header_id = sdh.source_deal_header_id
		LEFT JOIN source_deal_pnl_detail sdpd
			ON sdpd.source_deal_header_id = sdh.source_deal_header_id
		...
		LEFT JOIN source_remit_standard src_remit
			ON sdh.source_deal_header_id = src_remit.source_deal_header_id
		...
		LEFT JOIN static_data_value sdv_country
			ON sdv_country.value_id = sco.country
		...
		WHERE sub_cpty.LEI IS NOT NULL
			AND deal_status.code IN ('New', 'Amended', 'Cancelled')
			AND CASE WHEN NULLIF(@action_type, '') IS NULL THEN '1' 
					 ELSE CASE WHEN deal_status.code IN ('New', 'Amended') THEN 'N' 
							   WHEN deal_status.code = 'Cancelled' THEN 'C' 
						  END 
		...
		BEGIN TRY
			BEGIN TRAN
			INSERT INTO source_emir(source_deal_header_id, deal_id, sub_book_id, reporting_timestamp, counterparty_id, other_counterparty_id, counterparty_name, counterparty_country, 
									corporate_sector, nature_of_reporting_cpty, nature_of_reporting_cpty2, broker_id, reporting_entity_id, clearing_member_id, beneficiary_type_id, beneficiary_id, trading_capacity, counterparty_side,
			...
			FROM source_emir
			WHERE NULLIF(contarct_mtm_value, '') IS NULL 
			...
			FROM source_emir
			WHERE NULLIF(contarct_mtm_value, '') IS NOT NULL 
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'initial_margin_posted', 'initial_margin_posted should me positive value'
			FROM source_emir
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'variation_margin_posted', 'variation_margin_posted should me positive value'
			FROM source_emir
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'initial_margin_received', 'initial_margin_received should me positive value'
			FROM source_emir
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'variation_margin_received', 'variation_margin_received should me positive value'
			FROM source_emir
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'excess_collateral_posted', 'excess_collateral_posted should me positive value'
			FROM source_emir
			...
			INSERT INTO #temp_messages ([source_deal_header_id], [column], [messages])
			SELECT DISTINCT source_deal_header_id, 'excess_collateral_received', 'excess_collateral_received should me positive value'
			FROM source_emir
			WHERE excess_collateral_posted < 0
	...
	BEGIN
		...
		SELECT deal_id, 
			   response_status,
			   create_ts,
			   RANK() OVER(PARTITION BY deal_id ORDER BY create_ts DESC) [rank]
		INTO #mifid_log_status
		FROM source_mifid_audit_log
		ORDER BY deal_id
		...
		DELETE a
		--SELECT DISTINCT a.deal_id, response_status, sm.report_status, sdv.code
		FROM #temp_deals a
		...
		WHERE b.[rank] = 1
			AND sm.[rank] = 1
			AND (
					(	response_status NOT IN ('RJCT', 'RJPD') 
						AND sdv.code <> 'Cancelled'									
					) 
					OR (
						response_status IN ('RJCT', 'RJPD')
						AND sdv.code = 'Cancelled'						
					)
					OR (
						response_status NOT IN ('RJCT', 'RJPD') 
						AND sm.report_status = 'CANC'
						AND sdv.code = 'Cancelled'
					)
				)
		...
		SELECT DISTINCT
			   ...
			   report_status = CASE 
			   					...
			   					WHEN deal_status.code = 'Cancelled' THEN 'CANC' 
			   				END,
			   ...
			   buyer_id = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 'b', sub_cpty.LEI, deal_cpty.LEI), NULL),
			   buyer_country = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 'b', NULLIF(c_deal_cpty.code, ''), NULLIF(c_sub_cpty.code, '')), NULL),
			   buyer_fname = NULL,
			   buyer_sname = NULL,
			   buyer_dob = NULL,
			   buyer_decision_maker_code = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 'b',st.national_id,dm_cc.national_id)), NULL),
			   buyer_decision_maker_fname = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 'b',st.trader_name,dm_cc.[name])), NULL),
			   buyer_decision_maker_sname = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 'b',st.last_name,dm_cc.last_name)), NULL),
			   buyer_decision_maker_dob = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 'b',st.date_of_birth,dm_cc.date_of_birth), NULL),
			   seller_id = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 's', sub_cpty.LEI, deal_cpty.LEI), NULL),
			   --seller_country = IIF(deal_status.code IN ('New', 'Amended'), IIF(IIF(sdh.header_buy_sell_flag = 's', NULLIF(sub_cpty.LEI, ''), NULLIF(deal_cpty.LEI, '')) IS NOT NULL, 'NL', NULL), NULL),
			   seller_country = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 's', NULLIF(c_sub_cpty.code, ''), NULLIF(c_deal_cpty.code, '')), NULL),
			   seller_fname = NULL,
			   seller_sname = NULL,
			   seller_dob = NULL,
			   seller_decision_maker_code = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 's',st.national_id,dm_cc.national_id)), NULL),
			   seller_decision_maker_fname = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 's',st.trader_name,dm_cc.[name])), NULL),
			   seller_decision_maker_sname = IIF(deal_status.code IN ('New', 'Amended'), UPPER(IIF(sdh.header_buy_sell_flag = 's',st.last_name,dm_cc.last_name)), NULL),
			   seller_decision_maker_dob = IIF(deal_status.code IN ('New', 'Amended'), IIF(sdh.header_buy_sell_flag = 's',st.date_of_birth,dm_cc.date_of_birth), NULL),
			   order_trans_indicator = IIF(deal_status.code <> 'Cancelled', 'false', NULL),
			   ...
			   quantity_currency = NULL,--IIF(deal_status.code IN ('New', 'Amended'), sc.currency_name, NULL),
			   ...
			   net_amount = NULL,
			   venue = IIF(deal_status.code IN ('New', 'Amended'), IIF(scn.counterparty_id = gmv.clm7_value, gmv.clm4_value, 'XOFF'), NULL),
			   ...
			   instrument_id_code = IIF(deal_status.code IN ('New', 'Amended'), IIF(IIF(scn.counterparty_id = gmv.clm7_value, gmv.clm4_value, 'XOFF') <> 'XXXX', gmv1.clm1_value, NULL), NULL),
			   ...
			   underlying_instrument_code = IIF(deal_status.code IN ('New', 'Amended'), gmv1.clm4_value, NULL),
			   underlying_index_name = NULL,
			   underlying_index_term = NULL,
			   option_type = CASE 
			   				WHEN deal_status.code IN ('Cancelled') OR 
			   ...
			   option_exercise_style = CASE 
			   							...
			   							ELSE CASE WHEN UPPER(sdh.option_excercise_type) = 'A' THEN 'AMER' WHEN UPPER(sdh.option_excercise_type) = 'E' THEN 'EURO' END
			   						END,						
			   maturity_date = '',
			   expiry_date = CASE WHEN deal_status.code IN ('Cancelled') OR deal_udf.[Instrument Classification] LIKE 'E_____' OR 
			   						deal_udf.[Instrument Classification] LIKE 'C_____' OR deal_udf.[Instrument Classification] LIKE 'D_____' OR
			   				...
			   				END,
			   delivery_type = IIF(deal_status.code IN ('Cancelled'), NULL, 'CASH'),
			   firm_invest_decision = IIF(deal_status.code <> 'Cancelled', st.national_id, NULL),
			   decision_maker_country = IIF(deal_status.code <> 'Cancelled' AND st.national_id IS NOT NULL, 'NL', NULL),
			   firm_execution = IIF(deal_status.code <> 'Cancelled', st.national_id, NULL),
			   supervising_execution_country = IIF(deal_status.code <> 'Cancelled' AND st.national_id IS NOT NULL, 'NL', NULL),
			   ...
			   commodity_derivative_indicator = IIF(deal_status.code <> 'Cancelled', IIF(sco.commodity_id IN ('EUA', 'EUAA'), 'true', 'false'), NULL), -- EUA and EUAA are emmission allowence
			   securities_financing_transaction_indicator = IIF(deal_status.code <> 'Cancelled', 'false', NULL),
			   report_type = 1,
		...
		LEFT JOIN #temp_cpty_udf_values deal_cpty
			ON deal_cpty.source_deal_header_id = sdh.source_deal_header_id
		LEFT JOIN source_counterparty sc_deal_cpty ON sc_deal_cpty.source_counterparty_id = deal_cpty.counterparty_id
		LEFT JOIN static_data_value c_deal_cpty ON sc_deal_cpty.country = c_deal_cpty.value_id
		...
		LEFT JOIN #temp_cpty_udf_values sub_cpty
			ON sub_cpty.sub_book_id = sdh.sub_book_id
		LEFT JOIN source_counterparty sc_sub_cpty ON sc_sub_cpty.source_counterparty_id = sub_cpty.counterparty_id		
		LEFT JOIN static_data_value c_sub_cpty ON sc_sub_cpty.country = c_sub_cpty.value_id
		...
		WHERE deal_status.code IN ('New', 'Amended', 'Cancelled')
	...
	BEGIN
		DELETE b
		FROM #temp_deals a
		INNER JOIN source_mifid_trade b
			ON a.source_deal_header_id = b.source_deal_header_id
				AND a.deal_id = b.deal_id
				AND a.sub_book_id = b.sub_book_id
		...
		SELECT DISTINCT source_deal_header_id = sdh.source_deal_header_id,
						...
						instrument_identification_code = IIF(IIF(scn.counterparty_id = gmv.clm7_value, gmv.clm4_value, 'XOFF') <> 'XXXX', gmv1.clm1_value, NULL),
						...
						notional_amount = sdd.total_volume * sdd.fixed_price,
						...
						transaction_identification_code = sdh.deal_id,--IIF(gmv.clm14_value = 'Y' OR gmv.clm4_value NOT IN ('XXXX', 'XOFF'), deal_udf.[Trading Venue Transaction ID], NULL),
						...
						trade_report_trans_type = CASE WHEN @action_type_mifid = 'NEWT' THEN 0 WHEN @action_type_mifid = 'CANC' THEN 1 ELSE 2 END,
						package_id = NULL,
						trade_number = NULL,
						total_num_trade_reports = NULL,
						security_id = NULL,
						security_id_source = NULL,
						unit_of_measure = sdd.position_uom,
						contract_multiplier = CASE WHEN sco.commodity_id IN ('CER', 'EUA', 'ERU', 'EUAA') THEN 1000 
												   WHEN sco.commodity_id = 'Gasoil' THEN 100
												   WHEN sco.commodity_id = 'Rbob' THEN 42000
												   ELSE sdd.multiplier
											  END,
						reporting_party_lei = sub_cpty.LEI,
						submitting_party_lei = sub_cpty.LEI,
						submitting_party_si_status = 'Y',
						asset_class = 'Emission Allowance',
						...
						asset_sub_class = CASE WHEN sco.commodity_id = 'CER' THEN 'Certified Emission Reductions (CER)' 
										  ...
										  END,
						maturity_date = CASE WHEN @action_type_mifid = 'CANC' OR deal_udf.[Instrument Classification] LIKE 'E_____' OR 
												deal_udf.[Instrument Classification] LIKE 'C_____' OR deal_udf.[Instrument Classification] LIKE 'D_____' OR
												deal_udf.[Instrument Classification] LIKE 'T_____' OR deal_udf.[Instrument Classification] LIKE 'M_____' 
											THEN NULL 
											ELSE CONVERT(DATE, (ISNULL(hg.exp_date, sdd.contract_expiration_date)), 120)
						...
						exercise_style = CASE 
											WHEN (@action_type_mifid = 'CANC' OR
													deal_udf.[Instrument Classification] LIKE 'F_____' OR deal_udf.[Instrument Classification] LIKE 'S_____' OR 
													deal_udf.[Instrument Classification] LIKE 'E_____' OR deal_udf.[Instrument Classification] LIKE 'C_____' OR
													deal_udf.[Instrument Classification] LIKE 'C_____' OR deal_udf.[Instrument Classification] LIKE 'D_____' OR
													deal_udf.[Instrument Classification] LIKE 'I_____' OR deal_udf.[Instrument Classification] LIKE 'J_____' OR
						...
						underlying_maturity_date = CASE WHEN @action_type_mifid = 'CANC' OR deal_udf.[Instrument Classification] LIKE 'E_____' OR 
															deal_udf.[Instrument Classification] LIKE 'C_____' OR deal_udf.[Instrument Classification] LIKE 'D_____' OR
															deal_udf.[Instrument Classification] LIKE 'T_____' OR deal_udf.[Instrument Classification] LIKE 'M_____' 
														THEN NULL 
														ELSE CONVERT(DATE, (ISNULL(hg.exp_date, sdd.contract_expiration_date)), 120)
		...
		BEGIN TRY
			...
			INSERT INTO source_mifid_trade (
				...
				underlying_index_term, underlying_further_sub_product, underlying_other_security_type, underlying_other_further_sub_product, PreviouslyReported
			)
			...
			--INSERT INTO #temp_messages_mifid_trade ([source_deal_header_id], [column], [messages])
			--SELECT DISTINCT source_deal_header_id, 'venue_of_publication','For New Trade, Venue of Publication cannot be blank'
			--FROM source_mifid_trade 
			--WHERE NULLIF(venue_of_publication, '') IS NULL
			--	AND process_id = @process_id
			--	AND @action_type_mifid = 'NEWT'

			--INSERT INTO #temp_messages_mifid_trade ([source_deal_header_id], [column], [messages])
			--SELECT DISTINCT source_deal_header_id, 'publication_date_and_time','For New Trade, Publication Time Stamp cannot be blank'
			--FROM source_mifid_trade 
			--WHERE NULLIF(publication_date_and_time, '') IS NULL
			--	AND process_id = @process_id
			--	AND @action_type_mifid = 'NEWT'
			...
			--INSERT INTO #temp_messages_mifid_trade ([source_deal_header_id], [column], [messages])
			--SELECT DISTINCT source_deal_header_id, 'publication_date_and_time','Invalid Date Format for Publication Time Stamp'
			--FROM source_mifid_trade 
			--WHERE NULLIF(publication_date_and_time, '') IS NOT NULL
			--	AND publication_date_and_time NOT LIKE '____-__-__T__:__:__.______Z'
			--	AND process_id = @process_id
			--	AND @action_type_mifid = 'NEWT'
...
BEGIN
	...
	SET @_sql = '
		...
		WHERE (CONVERT(VARCHAR(10), sdh.create_ts, 120) BETWEEN ''' + @create_date_from + ''' AND ''' + @create_date_to + '''
				OR CONVERT(VARCHAR(10), sdh.update_ts, 120) BETWEEN ''' + @create_date_from + ''' AND ''' + @create_date_to + ''')
	...
	BEGIN
		SET @sql_str = '
			SELECT '''' [*Comments],				   
				   ...
				   ''NL'' [Country of the Other Counterparty - Trade Party 1 ],
				   ISNULL(counterparty_country, '''') [Country of the Other Counterparty - Trade Party 2],
				   ''F'' [Corporate sector of the reporting counterparty - Trade Party 1  ],
				   ISNULL(corporate_sector, '''') [Corporate sector of the reporting counterparty - Trade Party 2],
				   ISNULL(nature_of_reporting_cpty, '''') [Nature of the Reporting Counterparty - Trade Party 1],
				   ISNULL(nature_of_reporting_cpty2, '''') [Nature of the Reporting Counterparty - Trade Party 2],
				   '''' [Broker ID - Trade Party 1],
				   ISNULL(broker_id, '''') [Broker ID - Trade Party 2],
				   '''' [Clearing Member ID - Trade Party 1],
				   '''' [Clearing Member ID - Trade Party 2],
				   ISNULL(beneficiary_type_id, '''') [Type of ID of the Beneficiary - Trade Party 1],
				   ISNULL(beneficiary_id, '''') [Beneficiary ID - Trade Party 1],
				   ISNULL(beneficiary_type_id, '''') [Type of ID of the Beneficiary - Trade Party 2],
				   ISNULL(beneficiary_id, '''') [Beneficiary ID - Trade Party 2],
				   ISNULL(trading_capacity, '''') [Trading Capacity - Trade Party 1],
				   ISNULL(trading_capacity, '''') [Trading Capacity - Trade Party 2],
				   CASE WHEN ISNULL(counterparty_side, '''') = ''S'' THEN ''B'' ELSE ''S'' END [Counterparty Side - Trade Party 1],
				   ISNULL(counterparty_side, '''') [Counterparty Side - Trade Party 2],
				   '''' [Directly linked to commercial activity or treasury financing Indicator - Trade Party 1],
				   ISNULL(commercial_or_treasury, '''') [Directly linked to commercial activity or treasury financing Indicator - Trade Party 2],
				   '''' [Clearing Threshold - Trade Party 1 ],
				   ISNULL(clearing_threshold, '''') [Clearing Threshold - Trade Party 2 ],
				   ...
				   ISNULL(product_classification_type, '''') [Product Classification Type],
				   ISNULL(product_classification, '''') [Product Classification],
				   ISNULL(product_identification_type, '''') [Product identification type],
				   ISNULL(product_identification, '''') [Product identification],
				   ISNULL(underlying, '''') [Underlying identification type],
				   ISNULL(underlying_identification, '''') [Underlying identification],
				   ISNULL(notional_currency_1, '''') [Notional Currency 1],
				   ISNULL(notional_currency_2, '''') [Notional Currency 2],
				   ISNULL(derivable_currency, '''') [Deliverable currency],
				   ISNULL(trade_id, '''') [Trade ID],
				   ISNULL(report_tracking_no, '''') [Report Tracking Number],
				   ISNULL(complex_trade_component_id, '''') [Complex Trade Component ID],
				   ISNULL(exec_venue, '''') [Venue of execution ],
				   ...
				   ISNULL(option_type, '''') [Option Type],
				   ISNULL(option_style, '''') [Option exercise style ],
...
BEGIN	
	SELECT
		...
		ISNULL(product_classification, '') AS [Product classification],
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @lei = a.static_data_udf_values FROM source_counterparty sc
				...
				INNER JOIN fas_subsidiaries fs ON fs.counterparty_id = sc.source_counterparty_id 
				INNER JOIN portfolio_hierarchy ph ON ph.entity_id = fs.fas_subsidiary_id
					AND ph.entity_id = -1 AND autd.field_id = 'lei'
	...
	BEGIN
		...
		ELSE
		BEGIN
			EXEC spa_message_board 'u', @user_name, NULL, 'MiFID Trade', 'No data to submit. PLease check <b>MiFID Trade Error Report</b>', '', '', 's', @job_name, NULL, @batch_process_id, NULL, NULL, NULL, 'y', 
									   'No data to submit.', 'spa_source_emir' , NULL, NULL,NULL, ''
		END
	END
