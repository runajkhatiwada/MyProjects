...
DECLARE @sub_id VARCHAR(8000) = NULL,
		...
		@batch_process_id VARCHAR(100) = '',
		@batch_report_param VARCHAR(500) = NULL
SELECT @sub_book_id='66,71,67,68,8,7,6,43,42,11,12,10,3,5,4,46,47,48,25,26,147,146,70,44,45,16,17,18,19,21,20,22,24,23,72,74,73,27,28,52,54,29,30,31,49,32,91,79,78,76,77,62,63,33,34,59,36,35,39,40,38,37,55,56,58,120,121,122,123,108,109,114,115,116,117,94,95,112,113,103,104,102,106,107,105,100,101,96,97,118,119,110,111,124,126,125,127,92,93,99,98,128,129,81,82,83,80,148,142,145,144,143,141,88,89,90,130,131,132,133,135,134,138,137,140,139,60,61,57,64,65,69,75,86,84,85,87',@sub_id='7,6,73,54,98,101,68,38,43,47,63',@stra_id='8,12,15,71,52,27,74,75,57,59,61,55,99,102,69,39,44,48,65,64',@book_id='46,104,10,31,11,9,34,19,50,32,17,18,16,51,13,14,22,21,23,35,20,72,53,42,29,28,30,37,88,89,82,85,86,77,84,80,81,79,91,87,83,90,76,78,92,58,60,62,56,100,103,70,93,94,95,96,97,41,40,45,49,66,67',@create_date_from='2017-04-11',@create_date_to='2019-04-12',@deal_id='12925',@counterparty_id=NULL,@contract_id=NULL,@commodity_id=NULL,@flag='i',@submission_type='44703',@action_type='N',@level='T',@action_type_mifid='NEWT',@level_mifid='X',@deal_ref_id=NULL,@deal_date_from='',@deal_date_to='',@valuation_date='2019-04-12'
--SELECT @sub_book_id='25,81',@sub_id=NULL,@stra_id='57',@book_id='58',@create_date_from='2018-10-08',@create_date_to='2018-10-09',@status='39500',@flag='s',@submission_type='44703',@level_mifid='X',@level='M'
--*****************Test Code Start*******************/
SET NOCOUNT ON
...
DECLARE @_sql VARCHAR(MAX), @error_spa VARCHAR(1000), @temp_path VARCHAR(500), @pnl_deals VARCHAR(MAX)
...
BEGIN	
	...
	--SELECT @create_date_from = DATEADD(DAY, 1, EOMONTH(sdh.create_ts, -1)),
	--	   @create_date_to = EOMONTH(sdh.create_ts)
	--FROM source_deal_header sdh
	--INNER JOIN source_deal_pnl pnl
	--	ON sdh.source_deal_header_id = pnl.source_deal_header_id
	--WHERE CONVERT(VARCHAR(10), pnl.pnl_as_of_date, 120) = @valuation_date
...
BEGIN
	...
	*******************************************/
	...
	IF @create_date_from IS NOT NULL AND @create_date_to IS NOT NULL AND @pnl_deals IS NULL
	...
	IF @pnl_deals IS NOT NULL
		SET @_sql = @_sql + ' AND sdh.source_deal_header_id IN ('+ @pnl_deals +') '
	
	IF @deal_ref_id IS NOT NULL
		SET @_sql = @_sql + ' AND sdh.deal_id LIKE ''%' + @deal_ref_id + '%''' 
	
	IF @pnl_deals IS NULL AND (NULLIF(@deal_date_from, '') IS NOT NULL AND NULLIF(@deal_date_to, '') IS NULL)
		SET @_sql = @_sql + ' AND CONVERT(VARCHAR(10), sdh.deal_date, 120) = ''' + CONVERT(VARCHAR(10), @deal_date_from, 120) + ''''
	
	IF @pnl_deals IS NULL AND (NULLIF(@deal_date_to, '') IS NOT NULL AND NULLIF(@deal_date_from, '') IS NULL)
		SET @_sql = @_sql + ' AND CONVERT(VARCHAR(10), sdh.deal_date, 120) < ''' + CONVERT(VARCHAR(10), @deal_date_to, 120) + ''''
	
	IF @pnl_deals IS NULL AND (NULLIF(@deal_date_to, '') IS NOT NULL AND NULLIF(@deal_date_from, '') IS NOT NULL)
		SET @_sql = @_sql + ' AND CONVERT(VARCHAR(10), sdh.deal_date, 120) BETWEEN ''' + CONVERT(VARCHAR(10), @deal_date_from, 120) + ''' AND ''' + CONVERT(VARCHAR(10), @deal_date_to, 120) + ''''
	...
	IF NOT EXISTS (
		...
		WHERE sdv.code IN ('New', 'Amended', 'Reviewed')
	)
	BEGIN
		EXEC spa_ErrorHandler -1, 'Regulatory Submission', 'spa_source_emir', 'Error', 'No valid deals found.', ''
		RETURN
	...
	BEGIN		
		...
		SELECT DISTINCT 
			source_deal_header_id = sdh.source_deal_header_id, -- done
			...
			--contarct_mtm_value = MAX(sdpd.und_pnl),
			...
			--notional_amount = IIF(@level = 'M', SUM(ABS(sdpd.und_pnl)),  (CASE WHEN MAX(scm.commodity_id) IN ('CER', 'EUA', 'ERU', 'EUAA', 'JETFUEL NWE CIF', 'JETFUEL SINGAPORE') THEN 1000 * AVG(sdd.fixed_price)
			--																					WHEN MAX(scm.commodity_id) IN ('Gasoil','ULSD 10ppm CIF Med Cg','Biodiesel RME / Gasoil diff', 'Biodiesel FAME0 / Gasoil diff') THEN 100 * AVG(sdd.fixed_price)
			--																					WHEN MAX(scm.commodity_id) = 'RBOB Gasoline' THEN 42000 * AVG(sdd.fixed_price)
			--																					WHEN MAX(scm.commodity_id) = 'FX' THEN (1/AVG(sdd.fixed_price)) * MAX(ISNULL(sdd.deal_volume, 0))
			--																					ELSE MAX(sdd.multiplier) * AVG(sdd.fixed_price)
			--																				END)),
			notional_amount = IIF(@level = 'M', SUM(ABS(sdpd.und_pnl)),  NULL),
			price_multiplier = CASE WHEN MAX(scm.commodity_id) IN ('CER', 'EUA', 'ERU', 'EUAA', 'JETFUEL NWE CIF', 'JETFUEL SINGAPORE') THEN 1000 
									WHEN MAX(scm.commodity_id) IN ('Gasoil','ULSD 10ppm CIF Med Cg','Biodiesel RME / Gasoil diff', 'Biodiesel FAME0 / Gasoil diff') THEN 100
									WHEN MAX(scm.commodity_id) = 'RBOB Gasoline' THEN 42000
			...
			commodity_base = CASE WHEN MAX(scm.commodity_id) = 'FX' THEN 'OT' ELSE 'EV' END,--CASE WHEN MAX(deal_udf.[Asset Class]) = 'CO' THEN 'EV' ELSE NULL END,
			commodity_details = CASE WHEN MAX(scm.commodity_id) = 'FX' THEN 'OT' ELSE 'EM' END,--CASE WHEN (CASE WHEN MAX(deal_udf.[Asset Class]) = 'CO' THEN 'EV' ELSE NULL END) = 'EV' THEN 'EM' ELSE 'OT' END,
			delivery_point = NULL,
		...
		LEFT JOIN #source_deal_pnl sdpd
		...
		OUTER APPLY (
			...
			left join source_price_curve spc1 on spc1.as_of_date = @valuation_date and spc1.source_curve_def_id = spcd.source_curve_def_id and spc1.maturity_date = sdd.term_start
			left join source_price_curve spc2 on spc2.as_of_date = @valuation_date and spc2.source_curve_def_id = spcd.proxy_source_curve_def_id and spc2.maturity_date = sdd.term_start
			left join source_price_curve spc3 on spc3.as_of_date = @valuation_date and spc3.source_curve_def_id = spcd.monthly_index and spc3.maturity_date = sdd.term_start
			left join source_price_curve spc4 on spc4.as_of_date = @valuation_date and spc4.source_curve_def_id = spcd.proxy_curve_id3 and spc4.maturity_date = sdd.term_start
			where dmd.source_deal_header_id = sdd.source_deal_header_id and @level = 'M'
				
		) deal_match_info
		LEFT JOIN (
			SELECT gmvm.mapping_table_id,
				   gmvm.clm1_value as_of_date,
				   gmvm.clm2_value [index],
				   gmvm.clm3_value strike_price,
				   gmvm.clm4_value call_put,
				   gmvm.clm5_value expiration_date,
				   gmvm.clm6_value market_price
			FROM generic_mapping_values gmvm
			INNER JOIN generic_mapping_header gmhm
				ON gmhm.mapping_table_id = gmvm.mapping_table_id
		...
		WHERE sub_cpty.LEI IS NOT NULL
			...
			AND deal_status.code IN ('New', 'Amended', 'Reviewed')
		...
		UPDATE #temp_source_emir
		SET quantity = quantity / price_multiplier

		UPDATE #temp_source_emir
		SET notional_amount = IIF(@level <> 'M', notional_amount * quantity,  notional_amount)
	...
	BEGIN
		...
		BEGIN TRY
			...
			UPDATE #temp_source_mifid_trade
			SET quantity = quantity/contract_multiplier

			UPDATE #temp_source_mifid_trade
			SET notional_amount = notional_amount * quantity
	...
	BEGIN
		SET @sql_str = '
			SELECT '''' [*Comments],				   
				   ...
				   ISNULL(corporate_sector, '''') [Corporate sector of the reporting counterparty - Trade Party 1  ],
				   ISNULL(corporate_sector2, '''') [Corporate sector of the reporting counterparty - Trade Party 2],
				   ...
				   ISNULL(counterparty_side, '''') [Counterparty Side - Trade Party 1],
				   CASE WHEN ISNULL(counterparty_side, '''') = ''S'' THEN ''B'' ELSE ''S'' END [Counterparty Side - Trade Party 2],
