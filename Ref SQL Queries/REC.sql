...
CREATE INDEX idx_final_table ON #final_table (source_deal_header_id)
...
SET @_sql_string = '
	...
	INNER JOIN source_deal_detail sdd
		ON ft.source_deal_header_id = sdd.source_deal_header_id
...
'+
CASE 
	WHEN @_certificate_expiration_date_from IS NOT NULL AND @_certificate_expiration_date_to IS NULL THEN ' AND CONVERT(VARCHAR(10), gc.contract_expiration_date, 120) = ''' + CONVERT(VARCHAR(10), @_certificate_expiration_date_from, 120) + ''''
	ELSE ''
END
+ 
CASE 
	WHEN @_certificate_expiration_date_to IS NOT NULL AND @_certificate_expiration_date_from IS NULL THEN ' AND CONVERT(VARCHAR(10), gc.contract_expiration_date, 120) < ''' + CONVERT(VARCHAR(10), @_certificate_expiration_date_to, 120) + ''''
	ELSE ''
END
+ 
CASE 
	WHEN @_certificate_expiration_date_to IS NOT NULL AND @_certificate_expiration_date_from IS NOT NULL THEN ' AND CONVERT(VARCHAR(10), gc.contract_expiration_date, 120) BETWEEN ''' + CONVERT(VARCHAR(10), @_certificate_expiration_date_from, 120) + ''' AND ''' + CONVERT(VARCHAR(10), @_certificate_expiration_date_to, 120) + ''''
	ELSE ''
END
+ 
CASE 
	WHEN @_vintage_year_id IS NOT NULL THEN ' AND ft.vintage IN (' + @_vintage_year_id + ')'
	ELSE ''
END
+ 
CASE 
	WHEN @_tier_id IS NOT NULL THEN ' AND ft.tier_value_id IN (' + @_tier_id + ')'
	ELSE ''
END
+ 
CASE 
	WHEN @_jurisdiction_id IS NOT NULL THEN ' AND ft.state_value_id IN (' + @_jurisdiction_id + ')'
	ELSE ''
END

SET @_sql_string = @_sql_string + ' 
	GROUP BY ft.source_deal_header_id, sdd.term_start, sdd.leg, ft.deal_id, gc.gis_cert_date, 
			 gc.contract_expiration_date, gc.gis_certificate_number_from, gc.gis_certificate_number_to, 
			 cert_entity.code, gc.certificate_number_from_int, gc.certificate_number_To_int, ISNULL(sdv.code, ft.vintage), 
			 ft.vintage, tier_type.code, ft.tier_value_id, jurisdiction.code, ft.state_value_id, sub_id, stra_id, book_id,
			 sub_book_id, sub, stra, book, sub_book
'
